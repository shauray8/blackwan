...
import time
import copy
from wan.modules.flash_attn.cute.flash_fwd_sm100 import FlashAttentionForwardSm100
from wan.modules.flash_attn.cute.interface import _flash_attn_fwd
...
def get_block_mask(
    ...
    local_attn_size=-1,
    causal=True  # new flag
):
    total_length = num_frames * frame_seqlen
    padded_length = math.ceil(total_length / 128) * 128 - total_length
    full_len = total_length + padded_length

    def attention_mask(b, h, q_idx, kv_idx):
        if not causal:
            # Your original non-causal block-diagonal
    ...
    return create_block_mask(
        ...
        Q_LEN=full_len, KV_LEN=full_len, 
...
def get_block_mask(
    ...
    # Vectorized block end assignment
    block_size = frame_seqlen * num_frame_per_block
    ...
    # Expand each block end to cover its range
    block_ids = torch.arange(full_len, device=device) // block_size
    block_ids = torch.clamp(block_ids, 0, num_blocks - 1)
    ends[:total_tokens] = block_ends[block_ids[:total_tokens]]
    ends[total_tokens:] = full_len  # pad region attends nowhere

    def attention_mask(b, h, q_idx, kv_idx):
        if local_attn_size == -1:
            return (kv_idx < ends[q_idx]) | (q_idx == kv_idx)
        else:
            window_start = ends[q_idx] - local_attn_size * frame_seqlen
            return ((kv_idx < ends[q_idx]) & (kv_idx >= window_start)) | (q_idx == kv_idx)
...
def to_cute_tensor(t: torch.Tensor) -> cute.Tensor:
    # Ensure contiguous and proper memory format
    t = t.contiguous()
    # Convert via DLPack (handles layout, dtype, device correctly)
    return from_dlpack(torch.utils.dlpack.to_dlpack(t))

@cute.jit
def block_causal_score_mod(score, b, h, q_idx, kv_idx, *, aux_tensors=None):
    ends = aux_tensors[0]        # Int32[full_seq_len]
    offset_tensor = aux_tensors[1]  # cute.Tensor of shape [1], Int32
    offset = offset_tensor[0]    # Access the single element with [0]
    q_rel = q_idx[0]
    q_global = q_rel + offset
    allowed = (kv_idx < ends[q_global]) | (q_global == kv_idx)
    neg_inf = score - score - 1e6  # typed -inf
    return cute.where(allowed, score, neg_inf)

    
class FlashWanCrossAttention(nn.Module):
    ...
    def forward(self, x, context, context_lens, crossattn_cache=None):
        ...
        # Use FA4 (assuming non-causal full attn to context; add score_mod if masked)
...
class CausalWanSelfAttention(nn.Module):
    ...
    def __init__(self,
        ...
        super().__init__()
        self.ends_tensor = ends_tensor  # can be None
        self.dim = dim
        self.num_heads = num_heads
        self.head_dim = dim // num_heads
    ...
    def forward(
        ...
        if kv_cache is None or block_mask is not None:
            ...
            is_new_block = (current_frame % 3 == 0) and (current_frame > 0)
            
            if is_new_block and self.local_attn_size != -1:
                kv_cache["k"].zero_()
                kv_cache["v"].zero_()
            ...
            roped_query = causal_rope_apply(q, grid_sizes, freqs, start_frame=current_frame).type_as(v)
            roped_key = causal_rope_apply(k, grid_sizes, freqs, start_frame=current_frame).type_as(v)
            ...
            # Compute window size (for score_mod logic)
            ...
            o_fa, _ = _flash_attn_fwd(
                ...
                aux_tensors=[
                    self.ends_tensor.to(q_fa.device),
                    torch.tensor([current_start], dtype=torch.int32, device=q_fa.device)
                ],
                return_lse=False,
                num_splits=-1,
            )
...
class CausalWanAttentionBlock(nn.Module):
    ...
    def __init__(self,
        ...
        self.cross_attn = FlashWanCrossAttention(dim, num_heads, (-1, -1), qk_norm, eps)
...
class CausalWanModel(ModelMixin, ConfigMixin):
    ...
    def __init__(self,
        ...
        self.ends_tensor = compute_ends_tensor(
            num_frames=96,               # or your max inference frames
            frame_seqlen=1560,
            num_frame_per_block=self.num_frame_per_block,
            device=torch.device("cuda"),  # will be moved to GPU by model.to()
        ).to(torch.int32)
        self.blocks = nn.ModuleList([
            CausalWanAttentionBlock(cross_attn_type, dim, ffn_dim, num_heads,
                                    local_attn_size, sink_size, qk_norm, cross_attn_norm, eps,ends_tensor=self.ends_tensor)
