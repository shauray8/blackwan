...
def pack_int4_to_int8(x_int4: torch.Tensor) -> torch.Tensor:
    ...
    assert x_int4.shape[-1] % 2 == 0
    x_lo = x_int4[..., ::2].to(torch.uint8) & 0x0F
    x_hi = ((x_int4[..., 1::2].to(torch.uint8) & 0x0F) << 4)
    return (x_lo | x_hi).to(torch.int8)

def unpack_int8_to_int4(x_packed: torch.Tensor, shape: torch.Size) -> torch.Tensor:
    x_uint8 = x_packed.to(torch.uint8)
    x_lo = x_uint8 & 0x0F
    x_hi = (x_uint8 >> 4) & 0x0F
    x_lo = x_lo.to(torch.int8)
    x_hi = x_hi.to(torch.int8)
    x_lo = torch.where(x_lo >= 8, x_lo - 16, x_lo)
    x_hi = torch.where(x_hi >= 8, x_hi - 16, x_hi)
    x = torch.empty(shape, dtype=torch.int8, device=x_packed.device)
    x[..., ::2] = x_lo
    x[..., 1::2] = x_hi
    return x

def quantize_to_fp4_e2m1(x: torch.Tensor):
    amax = x.abs().max()
    scale = amax / 7.5 + 1e-8
    x_int4 = torch.clamp(torch.round(x / scale), -8, 7).to(torch.int8)
    if x_int4.shape[-1] % 2 != 0:
        x_int4 = torch.cat([x_int4, torch.zeros_like(x_int4[..., :1])], dim=-1)
    return pack_int4_to_int8(x_int4), scale

def dequantize_from_fp4_e2m1(x_packed: torch.Tensor, scale: torch.Tensor, shape: torch.Size):
...
class GenerationSession:
    ...
    def generate_block(self, models):
        if self.block_idx > 0 and hasattr(self, 'fp4_kv_quant'):
            (k_packed, k_scale), (v_packed, v_scale) = self.fp4_kv_quant
            k_cache = dequantize_from_fp4_e2m1(k_packed, k_scale, self.kv_cache_shape)
            v_cache = dequantize_from_fp4_e2m1(v_packed, v_scale, self.kv_cache_shape)
            models.pipeline.kv_cache1 = [k_cache, v_cache]
        ...
        for block in models.pipeline.generator.model.blocks:
            if hasattr(block.self_attn, 'kv_cache_quant') and block.self_attn.kv_cache_quant is not None:
                ...
                k_shape = block.self_attn.kv_cache_shape
                v_shape = block.self_attn.kv_cache_shape
                k_cache = dequantize_from_fp4_e2m1(k_packed, k_scale, k_shape)
                v_cache = dequantize_from_fp4_e2m1(v_packed, v_scale, v_shape)
                block.self_attn.kv_cache = (k_cache, v_cache)
