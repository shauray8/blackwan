...
class GemmSm100(GemmSm90):
    ...
    def is_valid_dtypes(
        ...
        # Allow FP16 (A) × FP8 (B) → FP16 (D) with FP32 accumulator (Blackwell)
        if (
            a_dtype == cutlass.BFloat16
            and b_dtype in {cutlass.Float8E4M3FN, cutlass.Float8E5M2}
            and acc_dtype == Float32
            and d_dtype in {cutlass.Float16, cutlass.BFloat16}
        ):
            return True
    
        # Also allow homogeneous cases (FP16×FP16, etc.)
        if a_dtype == b_dtype:
            if a_dtype in {
                cutlass.Float16,
                cutlass.BFloat16,
                ...
                if acc_dtype == Float32 and d_dtype in {
                    cutlass.Float16, cutlass.BFloat16, Float32
                }:
                    return True
                elif acc_dtype == cutlass.Float16 and d_dtype in {cutlass.Float16, cutlass.BFloat16}:
                    return True
                elif acc_dtype == Int32 and d_dtype in {Int32, cutlass.Int8, cutlass.Uint8}:
                    return True
