...
def mlp_func(x, weight1, weight2, activation: str, fuse_grad_accum=False, tuned=True):
    if activation == "swiglu":
        # weight1: [2 * intermediate, hidden] → gate_up
        gate_up = linear_func(x, weight1, bias=None, fuse_grad_accum=fuse_grad_accum, tuned=tuned)
        # Split into gate and up
        gate, up = gate_up.chunk(2, dim=-1)
        # Apply SwiGLU: silu(gate) * up
        y = F.silu(gate) * up
        # Down projection
        out = linear_func(y, weight2, bias=None, fuse_grad_accum=fuse_grad_accum, tuned=tuned)
        return out
    else:
        # Original path for other activations
        preact, postact = linear_act_func(
            x, weight1, activation,
            store_preact=torch.is_grad_enabled(),
            fuse_grad_accum=fuse_grad_accum,
            tuned=tuned,
        )
        out = act_linear_func(
            preact, weight2, postact, activation=activation,
            fuse_grad_accum=fuse_grad_accum,
            tuned=tuned,
        )
        return out
...
class MLP(nn.Module):
    ...
    def forward(self, input: Tensor) -> Tensor:
        if (
            ...
            # Fallback for non-fused path
            y = self.fc1(input)
            if self.activation == "swiglu":
                gate, up = y.chunk(2, dim=-1)
                y = F.silu(gate) * up
            else:
                y = F.gelu(y)  # or other activation
            return self.fc2(y)
