...
def linear_fwd_convert_type(*tensors):
    ...
    for t in tensors:
        if t.dtype in (torch.float8_e4m3fn, torch.float8_e5m2):
            ...
            converted.append(t.to(dtype=autocast_dtype))
        else:
            converted.append(t)
...
class LinearFunc(torch.autograd.Function):
    ...
    def forward(cls, ctx, x, weight, bias=None, fuse_grad_accum=False):
        ...
        out = cls.matmul_fwd_fn(x, weight.T, bias=bias, tuned=False)
...
class Linear(nn.Linear):
    ...
    def forward(self, input: Tensor) -> Tensor:
        if input.is_cuda and self.in_features % 8 == 0 and self.out_features % 8 == 0:
            return linear_func(input, self.weight, self.bias, fuse_grad_accum=self.fuse_grad_accum, tuned=False)
        else:
