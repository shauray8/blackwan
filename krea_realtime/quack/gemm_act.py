...
class GemmActMixin(GemmDefaultEpiMixin):
    ...
    def epi_visit_subtile(
        ...
        if const_expr(params.act_fn is not None):
            # Check if activation is SwiGLU (assumes act_fn.__name__ == "swiglu")
            if const_expr(hasattr(params.act_fn, '__name__') and params.act_fn.__name__ == "swiglu"):
                # SwiGLU: split tRS_rD into [gate, up]
                total_size = cute.size(tRS_rD)
                assert total_size % 2 == 0, "SwiGLU requires even output size"
                half = total_size // 2
                
                # Create output fragment for SwiGLU result (half size)
                tRS_rPostAct = cute.make_fragment((half,), self.acc_dtype)
                
                if const_expr(self.arch < 100):
                    for i in cutlass.range(half, unroll_full=True):
                        gate = tRS_rD[i]
                        up = tRS_rD[i + half]
                        tRS_rPostAct[i] = params.act_fn(gate, up)
                else:
                    # Vectorized: process 2 elements at a time
                    for i in cutlass.range(half // 2, unroll_full=True):
                        gate0 = tRS_rD[2 * i]
                        up0 = tRS_rD[2 * i + half]
                        gate1 = tRS_rD[2 * i + 1]
                        up1 = tRS_rD[2 * i + 1 + half]
                        out0, out1 = params.act_fn((gate0, gate1), (up0, up1))
                        tRS_rPostAct[2 * i] = out0
                        tRS_rPostAct[2 * i + 1] = out1
            else:
                # Regular elementwise activation (ReLU, GeLU, etc.)
...
act_fn_map = {
    ...
    "swiglu": quack.activation.swiglu,  # ← ADD THIS LINE
}
