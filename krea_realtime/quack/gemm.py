...
def gemm(
    ...
    if tensor_infos["B"].dtype in [torch.float8_e4m3fn, torch.float8_e5m2]:
        assert tensor_infos["A"].dtype in [torch.float16, torch.bfloat16], \
        "FP8 weights require FP16/BF16 activations"
        assert tensor_infos["D"].dtype in [torch.float16, torch.bfloat16], \
        "FP8 GEMM output must be FP16/BF16"
    major_configs = {
        "A": ("m", "k", "l"),
        "B": ("n", "k", "l"),
        "D": ("m", "n", "l"),
    ...
    #print("#########################",tensor_infos["A"].dtype, tensor_infos["B"].dtype, acc_dtype, tensor_infos["D"].dtype, tensor_infos["A"].major, tensor_infos["B"].major,"##############")
